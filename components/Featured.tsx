...
const baseCards = [
  ...
  {
    title: "Spend $500 and receive 1,000 points",
    subtitle: "$450 to go",
    button: "REDEEM",
    likes: "+3.5K",
    redeemCode: null,
  ...
  {
    title: "Double Points on all facial oils and serums",
  ...
  {
    title: "Refer a friend and earn bonus points",
    subtitle: "+200 points per referral",
    button: "REFER NOW",
    likes: "+890",
    redeemCode: null,
    scrollTarget: null as string | null,
...
const communityItems = [
  ...
  { type: "review" as const, author: "Nina C.", text: "Earned enough points for a free full-size serum. The rewards program is genuinely great.", time: "2d ago", stars: 5, hasImage: false },
...
const eventItems = [
  {
    ...
    points: "+50 points for attending",
  },
  {
  ...
  },
  {
    ...
    description: "Be the first to experience our new London flagship. Live music, complimentary treatments, and 2x points on all purchases during opening weekend.",
    date: "Jun 7, 2026",
    location: "34 King's Road, London",
    points: "2x points all weekend",
  },
  {
    type: "digital" as const,
    ...
    points: "+30 points for voting",
  },
];

/* ─── Single event card ─── */
function EventCard({ event, image, index, isFirst }: { event: (typeof eventItems)[0]; image: string; index: number; isFirst?: boolean }) {
  ...
  return (
    ...
    >
      {/* Image */}
      <div style={{ width: "240px", minHeight: "200px", flexShrink: 0, overflow: "hidden", position: "relative" }}>
        <img
          src={image}
          alt=""
          style={{
        ...
        <div
          style={{
            ...
            color: event.type === "digital" ? "#1a1a1a" : "#ffffff",
            backgroundColor: event.type === "digital" ? "rgba(255,255,255,0.92)" : "rgba(0,0,0,0.7)",
            padding: "5px 10px",
            lineHeight: 1,
          }}
        >
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          {isFirst && (
            <span style={{ width: "6px", height: "6px", borderRadius: "50%", backgroundColor: "#2196f3", flexShrink: 0 }} />
          )}
          <span style={{ fontFamily: "var(--font-sans)", fontSize: "12px", fontWeight: 600, color: "#1a1a1a", letterSpacing: "0.02em" }}>
            {event.date}
          </span>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginTop: "8px" }}>
          ...
          <span style={{ fontFamily: "var(--font-sans)", fontSize: "12px", fontWeight: 500, color: "#999" }}>
            {event.points}
          </span>
        </div>
      </div>
...
function EventsFeed() {
  ...
  return (
    <div style={{ maxWidth: "1280px", margin: "0 auto" }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: "1px", backgroundColor: "#e5e2de", border: "1px solid #e5e2de" }}>
        {shown.map((event, i) => (
          <EventCard key={`${event.title}-${i}`} event={event} image={shuffledImages[i % shuffledImages.length]} index={i} isFirst={i === 0} />
        ))}
      </div>
...
/* ─── Community card ─── */
function CommunityCard({ item, image, index }: { item: (typeof communityItems)[0]; image: string; index: number }) {
  const [hovered, setHovered] = useState(false);
  ...
  return (
    <div
      ...
      style={{
        backgroundColor: "#ffffff",
        display: "flex",
        flexDirection: item.hasImage ? "row" : "column",
        overflow: "hidden",
        opacity: 0,
        transform: "translateY(12px)",
        animation: `communityFadeIn 0.4s ease ${index * 0.06}s forwards`,
    ...
    >
      {item.hasImage && (
        <div style={{ width: "200px", minHeight: "160px", flexShrink: 0, overflow: "hidden" }}>
          <img
            src={image}
            alt=""
            style={{
      ...
      <div style={{ padding: "24px 32px", display: "flex", flexDirection: "column", gap: "8px", flex: 1, justifyContent: "center" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <span style={{ fontFamily: "var(--font-sans)", fontSize: "11px", fontWeight: 600, letterSpacing: "0.05em", color: item.type === "instagram" ? "#c13584" : "#1a1a1a", textTransform: "uppercase" }}>
        ...
        {item.type === "review" && item.stars && (
          <div style={{ display: "flex", gap: "1px" }}>
            {Array.from({ length: 5 }).map((_, i) => (
              <span key={i} style={{ fontSize: "12px", color: i < item.stars! ? "#1a1a1a" : "#ddd" }}>★</span>
            ))}
          </div>
        )}
        <p style={{ fontFamily: "var(--font-serif)", fontSize: "16px", fontWeight: 400, lineHeight: 1.5, color: "#1a1a1a", margin: 0 }}>{item.text}</p>
...
function CommunityFeed() {
  ...
  return (
    <div style={{ maxWidth: "1280px", margin: "0 auto" }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: "1px", backgroundColor: "#e5e2de", border: "1px solid #e5e2de" }}>
        {shown.map((item, i) => (
          <CommunityCard key={`${item.author}-${item.time}-${i}`} item={item} image={shuffledImages[i % shuffledImages.length]} index={i} />
        ))}
      </div>
      <div style={{ display: "flex", justifyContent: "center", gap: "12px", marginTop: "28px" }}>
        {hasMore && (
          <button
            onClick={() => setVisibleCount((c) => Math.min(c + 4, shuffled.length))}
            style={{ fontFamily: "var(--font-sans)", fontSize: "12px", fontWeight: 600, letterSpacing: "0.04em", color: "#1a1a1a", backgroundColor: "transparent", border: "1px solid #1a1a1a", padding: "10px 28px", cursor: "pointer", textTransform: "uppercase", transition: "background-color 0.2s ease, color 0.2s ease" }}
            onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = "#1a1a1a"; e.currentTarget.style.color = "#fff"; }}
            onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = "transparent"; e.currentTarget.style.color = "#1a1a1a"; }}
          >
